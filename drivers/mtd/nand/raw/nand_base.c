				...
				const void *data)
{
	...
	chip->legacy.write_buf(chip, params, ONFI_SUBFEATURE_PARAM_LEN);
				...
				void *data)
{
	...
	chip->legacy.read_buf(chip,  params, ONFI_SUBFEATURE_PARAM_LEN);
...
//	printk("<%x,%x>", readlen, col);	/* ### */
	...
	while (1) {
		...
		if (realpage != chip->pagecache.page || oobbuf) {
			...
			if (realpage < skippage) {
				...
				ret = chip->ecc.read_page_raw(chip,
		    ...
		    int allowbbt)
{
	...
	if ((chip->options & NAND_SW_WRITE_PROTECT) || (instr->addr < mtd->skip))
		return -EROFS;
