/*
 * drivers/input/touchscreen/st1633i.c
 *
...
#include <linux/platform_data/st1633i.h>
...
static void sitronix_ts_reset_ic(void)
{
	...
	gpio_request(sitronix_ts_gpts.reset_pin, "Multitouch Reset");
	gpio_direction_output(sitronix_ts_gpts.reset_pin, 1);
	gpio_set_value(sitronix_ts_gpts.reset_pin, 0);
	msleep(1);
	gpio_set_value(sitronix_ts_gpts.reset_pin, 1);
...
{
	if(sitronix_ts_gpts.x_rev)
		x = sitronix_ts_gpts.resolution_x - x;
	if(sitronix_ts_gpts.y_rev)
		y = sitronix_ts_gpts.resolution_y - y;
#ifdef SITRONIX_SUPPORT_MT_SLOT
	input_mt_slot(input_dev, id);
	input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, true);
	if(sitronix_ts_gpts.xy_swap) {
...
static DEVICE_ATTR(reprobe, S_IWUSR, NULL, sitronix_ts_reprobe_store);
...
#ifdef CONFIG_OF
static int sitronix_ts_probe_dt(struct i2c_client *client)
{
	struct device_node *np = client->dev.of_node;

	sitronix_ts_gpts.reset_pin = of_get_named_gpio(np, "reset-gpio", 0);
	sitronix_ts_gpts.x_rev = of_property_read_bool(np, "x-rev");
	sitronix_ts_gpts.y_rev = of_property_read_bool(np, "y-rev");
	sitronix_ts_gpts.xy_swap = of_property_read_bool(np, "swap-xy");
...
static int sitronix_ts_probe(struct i2c_client *client)
{
	...
	ret = sitronix_ts_probe_dt(client);
	if (ret) {
