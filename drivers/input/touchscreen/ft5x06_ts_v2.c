...
/* Activating updates is dangerous, this only works for ony type of panel!!! */
#define CFG_SUPPORT_AUTO_UPG	0		/* Support firmware update */
#define CFG_SUPPORT_UPDATE_PROJECT_SETTING 0	/* Support config update */
...
#if  CFG_SUPPORT_AUTO_UPG || CFG_SUPPORT_UPDATE_PROJECT_SETTING
 ...
 * [parameters]:
 *    client[in]:	the i2c client of the ctpm
 *    buf[out]:		data buffer
 *    length[in]:	the length of the data buffer
 */
static void i2c_read_interface(struct i2c_client *client, FTS_BYTE *buf,
			       FTS_DWRD length)
{
	int ret;
	...
	if (ret <= 0)
		printk("[FTS] i2c_read_interface error: %d\n", ret);
}
 ...
 *    length[in]:	the length of the data buffer
 */
static void i2c_write_interface(struct i2c_client *client, FTS_BYTE *buf,
				FTS_DWRD length)
{
	int ret;
	ret = i2c_master_send(client, buf, length);
	if (ret <= 0)
		printk("[FTS] i2c_write_interface error: %d\n", ret);
}
 ...
 *    dw_length[in]:	the length of the FW + 6(the Head length)
...
static E_UPGRADE_ERR_TYPE fts_ctpm_fw_upgrade(struct i2c_client *client,
				       FTS_BYTE *pbt_buf, FTS_DWRD dw_length)
{
	...
	if (i_ret <= 0) {
		printk("[FTS] Failed to enter upgrade mode, error=%d\n", i_ret);
		return ERR_MODE;
	}
	...
	cmd_buf[2] = 0x00;
	cmd_buf[3] = 0x00;
	i2c_write_interface(client, cmd_buf, 4);
	i2c_read_interface(client, reg_val, 2);
	...
	cmd_buf[0] = 0xcd;
	i2c_write_interface(client, cmd_buf, 1);
	i2c_read_interface(client, reg_val, 1);
	...
	for (j = 0; j < packet_number; j++) {
		...
		packet_buf[4] = (FTS_BYTE)(length>>8);
		packet_buf[5] = (FTS_BYTE)length;
		...
		i2c_write_interface(client, &packet_buf[0], FTS_PACKET_LENGTH + 6);
	...
	if ((dw_length) % FTS_PACKET_LENGTH > 0) {
		...
		i2c_write_interface(client, &packet_buf[0], temp+6);
		mdelay(20);
	}
	...
	for (i = 0; i < 6; i++) {
		...
		i2c_write_interface(client, &packet_buf[0], 7);
		mdelay(20);
	}
...
static int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	mdelay(100);		  /* Wait to enter factory mode */
	ft5x0x_write_reg(client, 2, 0x4); /* Command to start calibration */
	mdelay(300);
	for (i = 0; i < 100; i++) {
...
static int fts_ctpm_auto_upg(struct i2c_client *client)
{
	...
	/* Get firmware version of touch panel */
	ft5x0x_read_reg(client, FT5X0X_REG_FIRMID, &uc_tp_fm_ver);

	/* Get firmware version of .i file */
	if (sizeof(CTPM_FW) <= 2)
		return 0;		    /* No firmware data available */
	uc_host_fm_ver = CTPM_FW[sizeof(CTPM_FW) - 2];

	if ((uc_tp_fm_ver == 0xa6) ||       /* the firmware in touch panel
					       maybe corrupted */
	    (uc_tp_fm_ver < uc_host_fm_ver) /* host has newer firmware */
	    )
	{
		msleep(100);
		printk("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
		       uc_tp_fm_ver, uc_host_fm_ver);
		i_ret = fts_ctpm_fw_upgrade(client, CTPM_FW, sizeof(CTPM_FW));
		if (i_ret == 0) {
...
{
	unsigned char uc_i2C_addr;	/* I2C slave address (8 bit) */
...
static int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	buf[2] = 0x78;
	buf[3] = 0x0;
	i2c_write_interface(client, buf, 4);
	i2c_read_interface(client, buf, FTS_SETTING_BUF_LEN);
	...
	printk("\n");
	i2c_write_interface(client, &packet_buf[0], FTS_SETTING_BUF_LEN + 6);
	mdelay(100);
...
#endif
#endif /* CFG_SUPPORT_AUTO_UPG || CFG_SUPPORT_UPDATE_PROJECT_SETTING */
