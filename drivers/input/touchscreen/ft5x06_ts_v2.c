 ...
 *    dw_length[in]:	the length of the data buffer
 * [return]:
 *    FTS_TRUE:		success
 *    FTS_FALSE:	fail
 */
			    ...
			    FTS_DWRD dw_length)
{
	...
	ret = i2c_master_recv(this_client, pbt_buf, dw_length);
...
{
	int ret;
	ret = i2c_master_send(this_client, pbt_buf, dw_length);
	if (ret <= 0) {
 ...
 *    num[in]:		the valid input parameter numbers; if only command
 *			code needed and no parameters follow, then num is 1
 * [return]:
 *    FTS_TRUE:		success
 *    FTS_FALSE:	fail
 */
...
int fts_ctpm_auto_clb(void)
{
	...
	delay_qt_ms(100);		/* Make sure already enter factory
					   mode */
...
int fts_ctpm_auto_upg(void)
{
	...
	if ((uc_tp_fm_ver == 0xa6) ||       /* the firmware in touch panel
					       maybe corrupted */
	    (uc_tp_fm_ver < uc_host_fm_ver) /* the firmware in host flash is
					       new, need upgrade */
	    )
	{
		msleep(100);
...
static void ft5x0x_report_value(void)
{
	...
	for (i = 0; i < CFG_MAX_TOUCH_POINTS; i++) {
		if ((event->au16_x[i] < SCREEN_MAX_X)
			...
			/* LCD view area */
			down = ((event->au8_touch_event[i] == 0)
...
static void ft5x0x_ts_set_defaults(struct ft5x0x_ts_data *tsdata)
{
