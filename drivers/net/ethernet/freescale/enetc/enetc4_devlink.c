...
static int enetc4_devlink_reload_down(struct devlink *devlink, bool netns_change,
...
static int enetc4_devlink_parse_rings_param(union devlink_param_value *val,
					    u8 *si_num_rings, u32 *ring_cnt)
{
	...
	for (i = num_step - 1, k = 0; i >= 0; i--) {
		...
		for (j = 0; j < 8 && k < ENETC_MAX_SI_NUM - 1; j++) {
			num_ring = param_val & 0xff;
			si_num_rings[k++] = num_ring;
			*ring_cnt += num_ring;
			param_val >>= 8;
		}
...
static void enetc4_devlink_get_rings_param(struct devlink *devlink)
{
	struct enetc_devlink_priv *devl_priv = devlink_priv(devlink);
	struct enetc_pf *pf = devl_priv->pf;
	...
	err = enetc4_devlink_parse_rings_param(&val, &devl_priv->si_num_rings[vf_base_id],
					       &ring_cnt);
	if (err)
		goto clear_si_num_rings;

	/* Calculate the number of rings of PF */
	if ((rings_cap - ring_cnt) > ENETC_SI_MAX_RINGS_NUM)
		num_ring = ENETC_SI_MAX_RINGS_NUM;
	else
		num_ring = rings_cap - ring_cnt;

	devl_priv->si_num_rings[0] = num_ring;
...
static void enetc4_devlink_get_params(struct devlink *devlink)
{
	devl_assert_locked(devlink);

	enetc4_devlink_get_rings_param(devlink);
}

static int enetc4_devlink_reload_up(struct devlink *devlink,
				    enum devlink_reload_action action,
				    enum devlink_reload_limit limit,
				    ...
				    struct netlink_ext_ack *extack)
{
	...
	case DEVLINK_RELOAD_ACTION_DRIVER_REINIT:
		*actions_performed = BIT(DEVLINK_RELOAD_ACTION_DRIVER_REINIT);
		enetc4_devlink_get_params(devlink);
...
static const struct devlink_ops enetc_devlink_ops = {
	...
	.reload_down = enetc4_devlink_reload_down,
	.reload_up = enetc4_devlink_reload_up,
};

static void enetc4_devlink_free(void *devlink)
{
...
int enetc4_devlink_alloc(struct enetc_pf *pf)
{
	...
	err = devm_add_action_or_reset(dev, enetc4_devlink_free, devlink);
	if (err)
		return err;
...
static int enetc4_devlink_ring_num_validate(struct devlink *devlink, u32 id,
					    union devlink_param_value val,
					    struct netlink_ext_ack *extack)
{
	struct enetc_devlink_priv *devl_priv = devlink_priv(devlink);
	u8 vf_num_rings[ENETC_MAX_SI_NUM - 1] = {0};
	struct enetc_pf *pf = devl_priv->pf;
	u32 rings_cap = pf->caps.num_rx_bdr;
	...
	err = enetc4_devlink_parse_rings_param(&val, vf_num_rings, &ring_cnt);
	if (err) {
		NL_SET_ERR_MSG_MOD(extack, "Parameter format is error");
		return -EINVAL;
	}

	for (i = 0; i < ENETC_MAX_SI_NUM - 1; i++) {
		if (i < num_vf) {
			if (!vf_num_rings[i]) {
...
static const struct devlink_param enetc_devlink_params[] = {
	DEVLINK_PARAM_DRIVER(ENETC_DEVLINK_PARAM_ID_RING_NUM_ASSIGN,
			     ...
			     NULL, NULL, enetc4_devlink_ring_num_validate),
};
